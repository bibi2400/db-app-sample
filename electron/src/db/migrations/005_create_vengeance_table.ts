...
export class CreateVengeanceTableMigration extends DbMigrationDefinition {
  readonly id = '005_create_vengeance_table';
  ...
  async up(dataSource: DataSource): Promise<void> {
    ...
    await dataSource.query(`
        CREATE TABLE IF NOT EXISTS vengeanceTable (
    ...
    await dataSource.query(`
      INSERT INTO vengeanceTable (${headers.map((header) => header.key).join(', ')})
      VALUES ${values.join(', ')}
    `);
  }
}
